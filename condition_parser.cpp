...
shared_ptr<Node> ParseComparison(It& current, It end) {
  if (current == end) { // if comparison is empty throw logic error
    throw logic_error("Expected column name: date or event");
  }

  Token& column = *current; // remember pointer
  ...
  if (current == end) {
    throw logic_error("Expected comparison operation"); // if comparison is empty throw logic error
  }

  Token& op = *current; // remember pointer to operator
  if (op.type != TokenType::COMPARE_OP) {
    throw logic_error("Expected comparison operation"); // if no comparison operator throw logic error
  }
  ++current;

  if (current == end) {
    throw logic_error("Expected right value of comparison"); // if no value of comparison throw logic error
  }

  Comparison cmp; // fetch value of comparison operator
  if (op.value == "<") {
    cmp = Comparison::Less;
  } else if (op.value == "<=") {
  ...
  const string& value = current->value; // fetch value to compare
  ++current;

  if (column.value == "date") { // check whether events or dates are compared
...
shared_ptr<Node> ParseCondition(istream& is) {
  ...
  if (!top_node) { //check is top node is null pointer
